...
class MediumNativeAdView: NativeAdView {

    // Remove @IBOutlet declarations - these properties are inherited from NativeAdView
    // The parent class already provides: mediaView, iconView, headlineView, bodyView, callToActionView
    ...
    func configure(with nativeAd: NativeAd) {
        ...
        // Configure media view - cast to MediaView
        if let adMediaView = mediaView as? MediaView {
...
extension MediumNativeAdView {
    static func fromNib() -> MediumNativeAdView? {
        let bundle = Bundle(for: MediumNativeAdView.self)
        let nib = UINib(nibName: "MNativeView", bundle: bundle)
        return nib.instantiate(withOwner: nil, options: nil).first as? MediumNativeAdView
    }
}